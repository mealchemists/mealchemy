...
class MealPlanAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if not start_date or not end_date:
            ...
            return Response(
                {"meal_plan": meal_plan_serializer.data}, status=status.HTTP_200_OK
            )
        else:
            try:
            ...
            queryset = MealPlan.get_meals_for_range(
    ...
    def post(self, request, *args, **kwargs):
        for meal_plan_data in request.data:
            meal_plan_data["day_planned"] = datetime.strptime(
                meal_plan_data["day_planned"], "%Y-%m-%dT%H:%M:%S.%fZ"
            ).date()
            meal_plan_id = meal_plan_data.get(
                "id", None
            )  # Get the meal_plan ID from the request
